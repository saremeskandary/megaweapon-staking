...
contract WEAPON is Context, IWEAPONStakable {
  ...
  uint256 public mintLockTime = 1643673599;
  ...
  address payable private immutable _multiSig;
  ...
  constructor(address vault, address multiSig) {
    require(
      vault != address(0) && multiSig != address(0),
      "$WEAPON: cannot assign privilege to zero address"
    );
    _lp = _msgSender();
    ...
    )
  {
    if (stakingEnabled && _stakedBalances[account] > 0)
      return (
        ...
        _stakeBeginTime[account],
        _stakeExpireTime[account]
      );
    else return (0, 0, 0);
  }
  ...
  function _transfer(
    ...
    require(
      ...
      "$WEAPON: cannot transfer pool to pool"
    );
    ...
    if (
      _isPool[sender] == true && recipient != _lp && recipient != _uniRouter
    ) {
  ...
  function stake(
    ...
    require(
      unstakeTime > (block.timestamp + 86100),
      "$WEAPON: minimum stake time 23 hours 55 min"
    );
    require(
      unstakeTime >= _stakeExpireTime[account],
      "$WEAPON: new stake time cannot be shorter"
    );
    require(
      _balances[account] >= amount,
      "$WEAPON: stake exceeds available balance"
    );
  ...
  function unstake(
    ...
    require(
      _msgSender() == _stakingContract,
      "$WEAPON: must unstake through staking contract"
    );
    ...
    require(
      unstakeAmount <= _stakedBalances[account],
      "$WEAPON: unstake exceeds staked balance"
    );
  ...
  function mint(uint256 amount, address recipient) external onlyMultiSig {
    require(
      block.timestamp > mintLockTime && mintLocked == false,
      "$WEAPON: still frozen"
    );
  ...
  function toggleStaking() external override onlyMultiSig {
    require(
      ...
      "$WEAPON: staking contract not set"
    );
  ...
  function _tokensToETH(uint256 amount) private lockSwap {
    ...
    if (address(this).balance > 0) {
      if (stakingEnabled) {
        uint256 stakingShare = (address(this).balance * _stakingRewards) / 100;
